...
export class RegistrationComponent implements OnInit {
  ...
  base64Data:string = this.defaultBase64Data;;
  base64ContentType:string = this.defaultBase64ContentType;
  /* old one
  onFileChange(event) {
  ...
  */
  ...
  async  register() {
    ...
    let role = this.registrationForm.get('userType').value;
    ...
    const responseEmail = await firstValueFrom(this.registrationService.uniqueEmail(email));
    ...
    if (role === 'client') {
      this.registrationService.registerClient(username, password, email, role, firstname, lastname, phoneNumber, this.base64Data, this.base64ContentType).subscribe(resp => {
