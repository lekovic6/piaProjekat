...
export class RegistrationComponent implements OnInit {
      ...
      reader.onload = () => {
        ...
        setTimeout(() => {
          ...
          if( width < 100 || width > 300 ||  height < 100 || height > 300 ) {
            this.dimensionErrorMassage = "Invalid dimensions. Image dimensions should be between 100x100 and 300x300 pixels.";
  ...
  onFileChange(event) {
    ...
    if (!this.dimensionErrorFlag){
      ...
      if (file) {
        this.base64ContentType = file.type;
      }
    }
    else{// bad picture dimensions
      this.base64Data = this.defaultBase64Data;
      this.base64ContentType = this.defaultBase64ContentType;
    }
  ...
  ngOnInit(): void {
    this.registrationForm = this.formBuilder.group({
      ...
      // only agency
      agencyName: [''], agencyAdress: [''], tid: [''], description: [''],
    }, {
      validator: this.passwordMatchValidator // Custom validator function
    });

    // Add conditional validation for client fields
    this.registrationForm.get('userType').valueChanges.subscribe(userType => {
      const clientControls = ['firstname', 'lastname', 'phoneNumber'];

      if (userType === 'client') {
        clientControls.forEach(control => {
      ...
      // Update the validation status for the client fields
      clientControls.forEach(control => {
        this.registrationForm.get(control).updateValueAndValidity();
      });
    });

    // Add conditional validation for agency fields - eventualno da dodam pattern za broj telefona
    this.registrationForm.get('userType').valueChanges.subscribe(userType => {
      const agencyControls = ['agencyName', 'agencyAdress', 'tid', 'description'];
      ...
      // Update the validation status for the agency fields
      agencyControls.forEach(control => {
        this.registrationForm.get(control).updateValueAndValidity();
      });
    });
  ...
  register() {
    let userType = this.registrationForm.get('userType').value;
    ...
    let agencyName = this.registrationForm.get('agencyName').value;
    ...
    if (userType === 'client') {
      ...
      this.registrationService.registerClient(username, password, email, firstname, lastname, phoneNumber, this.base64Data, this.base64ContentType).subscribe(resp => {
      ...
      this.registrationService.registerAgency(username, password, email, agencyName, agencyAdress, tid, description);
    }
